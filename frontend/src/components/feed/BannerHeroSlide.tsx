      ...
      <div className="absolute inset-0">
        {banner.image ? (
          <Img
            ...
            /*
             * Рано — да, высшим приоритетом — нет.
             *
             * Комментарий выше говорил, что герой и есть LCP ленты. Замер
             * 12.09 говорит иначе: LCP — фотография первой записи
             * (54040 px², `ef6d4467…/medium.webp`), а баннер уходил ниже.
             * При этом `fetchpriority=high` стояло на обоих, и на канале
             * 1,6 Мбит/с они делили полосу: LCP-картинка весом 30 КБ
             * скачивалась 2258 мс, потому что рядом стартовали ещё 14
             * изображений на 238 КБ. Высший приоритет должен быть у одного —
             * у измеренного кандидата.
             */
            priority={false}
