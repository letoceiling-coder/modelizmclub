...
function Body({ value, onChange, onReset }: Props) {
  ...
  /*
   * Фильтр показывает тот же список, что форма подачи и каталог.
   *
   * До 17.09 здесь прятались разделы без объявлений: зеркало заводило в
   * каталоге узел на каждое направление, включая «Выставки и события» и
   * «Обзоры наборов», где продавать нечего, и выбор такого пункта был
   * тупиком. Теперь такие разделы в дерево объявлений не попадают вовсе
   * (флаг in_listings), а прятать пустые значило бы снова развести списки:
   * в форме раздел есть, в фильтре нет.
   */
