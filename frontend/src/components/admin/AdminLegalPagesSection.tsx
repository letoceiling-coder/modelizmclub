  ...
  return (
    <div style={{ display: "grid", gap: 16 }}>
      ...
      <table className="w-full text-sm" style={{ borderCollapse: "collapse" }}>
        ...
        <tbody>
          {pages.map((p) => (
            <tr key={p.id} style={{ borderBottom: "1px solid var(--border)", background: p.slug === FEATURED_SLUG ? "var(--accent-soft)" : undefined }}>
        ...
        <div className="rounded-lg border p-4" style={{ borderColor: "var(--border)" }}>
          ...
          <div className="grid gap-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Button type="button" size="sm" variant={mode === "html" ? "default" : "outline"} onClick={() => { setMode("html"); setSourceMode("html"); }}>
                HTML
              </Button>
              <Button type="button" size="sm" variant={mode === "markdown" ? "default" : "outline"} onClick={() => { setMode("markdown"); setSourceMode("markdown"); }}>
                Markdown
              </Button>
              <Button type="button" size="sm" variant={mode === "preview" ? "default" : "outline"} onClick={() => void openPreview()}>
                Превью
              </Button>
            </div>
            {mode === "html" && (
            ...
            {mode === "preview" && (
              <div className="legal-document min-h-[200px] rounded-md border p-4" dangerouslySetInnerHTML={{ __html: previewHtml }} />
            )}
          </div>
          <div className="mt-3 flex flex-wrap gap-2">
            {creating ? (
              <Button type="button" disabled={createMut.isPending || !draft.slug || !draft.title || (!draft.content_html && !draft.content_md)} onClick={() => createMut.mutate()}>
                Создать черновик
              </Button>
            ) : (
              <>
            ...
            <Button type="button" variant="ghost" onClick={() => { setEditing(null); setCreating(false); setShowHistory(false); }}>
              Закрыть
            </Button>
          </div>
          ...
          {editing && showHistory && (
            <div className="mt-4 rounded-md border p-3" style={{ borderColor: "var(--border)" }}>
              ...
              {revisionsQuery.isLoading ? (
                <p className="text-xs" style={{ color: "var(--foreground-50)" }}>Загрузка…</p>
              ) : (revisionsQuery.data ?? []).length === 0 ? (
                <p className="text-xs" style={{ color: "var(--foreground-50)" }}>Пока нет сохранённых версий. Они появятся после первого изменения.</p>
              ) : (
