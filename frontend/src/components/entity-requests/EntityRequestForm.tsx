...
export function EntityRequestForm({ kind, onClose, onSubmitted }: Props) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(
        already ? "У вас уже есть заявка на рассмотрении" : "Не удалось отправить заявку",
      );
      setSubmitting(false);
    }
  };
  return (
    <>
            ...
            {kind === "channel" && (
              <>
                <label className="flex flex-col gap-1">
                  ...
                  <div
                    className="flex items-center gap-1 rounded-[10px] border px-3"
                    style={inputStyle}
                  >
                    <span
      ...
      <PhotoEditorDialog
        ...
        onCropped={(blob) => {
          applyCropped(blob, "channel-avatar", setPendingAvatar, setAvatarPreview, () =>
      ...
      <PhotoEditorDialog
        ...
        onCropped={(blob) => {
          applyCropped(blob, "channel-banner", setPendingBanner, setBannerPreview, () =>
