  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-3 sm:items-center" onClick={() => onOpenChange(false)}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-sm rounded-2xl bg-card p-4 shadow-xl">
        <h3 className="font-display text-base font-semibold">{t("components.chooserTitle")}</h3>
        <div className="mt-4 grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span className="text-sm font-medium">{t("components.chooserPost")}</span>
          </button>
          <button
          ...
          >
            ...
            <span className="text-sm font-medium">{t("components.chooserAd")}</span>
          </button>
        </div>
      </div>
    </div>
