...
function ReviewsPage() {
  ...
  return (
    /*
      Без правой панели: центр на всё место до правого края, как у каталога
      (правило в `lib/layout/rails.ts`). Прыжка при переходе с ленты нет —
      левый край центра у обеих раскладок один, см. `AppLayout`.
    */
    <AppLayout>
      <div className="mx-auto flex max-w-[1200px] flex-col gap-[20px]">
        ...
        {/*
          ...
          Высоты — не на глаз. Герой держит пропорцию 16 : 7 (`ReviewsHero`),
          и заглушка берёт ту же пропорцию: с 11.09 у обзоров нет правой
          панели, центр от 351 до 1176 px, и герой от 154 до 515 по высоте —
          одно число здесь было бы верным на одной ширине. Ряд новинок —
          карточки постоянного размера, 273 на всех ширинах (замер 11.09
          на 375, 768, 1024, 1280, 1440, 1920).

          Каждая заглушка стоит на месте своего блока и уходит по ответу
          своего запроса: герой — по подборкам, «Новинки» — по сетке.
        */}
        {reserveHero ? (
          <Skeleton className="w-full rounded-[var(--r-card)]" style={{ aspectRatio: "16 / 7" }} />
        ) : (
