...
function MyAdsPage() {
  ...
  useObjectReload(
    ...
    fetchMyListings,
    setItems,
    "перечитывание списка объявлений после живого обновления",
  );
  ...
  /*
   * Публикация черновика может упереться в оплату.
   *
   * В платной категории сервер отвечает 422 «Для публикации объявления нужна
   * оплата», и до 21.09 всё заканчивалось откатом и тостом: заплатить из
   * списка нечем, а форма правки к оплате не вела. Черновик, который мастер
   * заводит сам перед уходом на банковскую форму, не публиковался уже
   * никогда (приёмка 20.09).
   *
   * Теперь отказ у черновика ведёт в форму правки: там считается котировка и
   * стоит кнопка «Оплатить … ₽ и опубликовать». Текст отказа человек видит
   * тостом — он и объясняет, почему его перенесли.
   *
   * Переносим при любом отказе черновика, а не только при «нужна оплата»:
   * второй возможный отказ здесь — «не удалось списать бесплатное
   * размещение, попробуйте ещё раз», и повторять попытку удобнее там же.
   * Различать их по тексту сообщения — худший способ из возможных.
   */
  const doPublish = (id: string) => {
    const previous = items.find((x) => x.ad.id === id)?.status ?? "archived";
    setLocalStatus(id, "active");
    publishListing(id).catch((error: unknown) => {
      withRollback(id, previous, "Не удалось опубликовать объявление")(error);
      if (previous === "draft") void navigate({ to: "/ads/new", search: { edit: id } });
    });
  };
