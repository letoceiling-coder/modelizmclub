/*
 * Настройка `footer.contacts` — единственный источник реквизитов и контактов
 * оператора. У неё три представления, и каждое отвечает на свой вопрос:
 *
 *   `legal/LegalRequisites`      — мелкая серая строка внизу подвала и
 *                                  лендинга: наименование, ИНН, ОГРН, адрес,
 *                                  почта. Нужна на каждой странице, поэтому
 *                                  набрана 11 px и не кричит.
 *   `layout/FooterContactsBlock` — колонка «Контакты» в подвале и на
 *                                  лендинге: почта, телефон, режим работы,
 *                                  соцсети. Способы связаться, без реквизитов.
 *   `legal/OperatorContacts`     — блок на `/legal/contacts`: все поля
 *                                  подписанной таблицей. Туда человек
 *                                  приходит специально за реквизитами, и
 *                                  статья 9 ЗоЗПП требует их вместе.
 *
 * Три вида одного источника — это нормально. Второй источник — нет: разойдутся,
 * и потом не выяснить, какой из двух верный. Всё правится из админки.
 */
