...
export async function sendPhoneVerificationCode(phone: string): Promise<{ expires_in_minutes: number }> {
...
export async function verifyPhoneCode(phone: string, code: string): Promise<import("@/lib/mock").User> {
  if (isDemoMode()) {
    const { DEMO_USER } = await import("@/lib/demo-data");
    return { ...DEMO_USER, phone, phone_verified: true };
  }
  const res = await api<{ data: import("@/lib/api/auth").ApiUser }>(
