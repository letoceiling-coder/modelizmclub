...
export function InlineFeedbackHost() {
  ...
  /*
   * Первый кадр в браузере обязан совпасть с серверным.
   *
   * Здесь до 22.09 стояло `if (typeof document === "undefined") return null`
   * — ветка «сервер/браузер», ровно та, что запрещена. Сервер рисовал на
   * этом месте пустоту, браузер сразу портал, и дальше всё дерево корня
   * съезжало: React сравнивал наш `<div role="status">` с `<section>`
   * тостов, стоящим следом, и бросал #418 — полную перерисовку страницы.
   * Замер 22.09: расхождение на каждом проверенном маршруте, и на проде
   * тоже, включая главную и ленту, у гостя.
   *
   * `useHydrated()` возвращает false и на сервере, и в кадре гидрации —
   * то есть оба рисуют пустоту и сходятся. Портал появляется кадром позже.
   * Отклику это ничем не мешает: он и так приходит по нажатию, то есть
   * заведомо после гидрации.
   */
  const готов = useHydrated();
  if (!готов) return null;

  return createPortal(
    /*
     * Живая область смонтирована всё время, пока приложение работает, —
     * меняется только текст внутри. Область, вставленная в дерево вместе
     * с содержимым, экранным диктором не объявляется: так «Добавлено в
     * избранное» для незрячего пропало бы совсем — у карточки каталога
     * другого сообщения об этом нет.
     */
    <div
      role="status"
      aria-live="polite"
