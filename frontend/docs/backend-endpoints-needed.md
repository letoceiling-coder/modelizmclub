...
- **Реквизиты выплат** (вторая половина Stage 2 — перенос
  `/settings/requisites` с localStorage на сервер с шифрованием) —
  **заблокировано вопросом к Никите**: способ вывода на старте (API-выплаты
  на карту через эквайринг vs ручные переводы). От ответа зависят поля и сам
  флоу, поэтому UI пока не мигрируется. `/settings/requisites` остаётся на
  localStorage до решения.
