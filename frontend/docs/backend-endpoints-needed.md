...
## Сводка для бэкенд-разработчика (обновлено 2026-07-16)
...
**Статус интеграции (2026-07-16):** весь backend-контракт из этого документа
**реализован на master**. Открытых backend-задач для фронта **нет** — только
`Infra` (ключи эквайринга на проде) и `Deferred` (delivery UI, anti-scraping,
CDN/транскодинг). Всё остальное — `Frontend-only`.

**Обновление 2026-07-16 (верификация + QA-fix):**
- **PHPUnit 103/103** на изолированной PostgreSQL БД `modelizmclub_test`
  (`deploy/scripts/setup-test-db.sh`, `run-backend-tests.sh`; `RefreshDatabase` +
  CRUD Feature-тесты: auth, listings, chat, friends, feed, billing, admin, icons,
  communities, delivery, escrow, seller-cabinet).
- **§3 hide message** — исправлен 500 (`message_user_hides` composite PK →
  `updateOrInsert` в `ChatService`); тест `ChatFrontendIntegrationTest`.
- **Мессенджер: вложения chat** — `GET /media/{uuid}` для `purpose=chat`;
  транскрипт голосовых для получателя (`TranscribeMediaController`).
- **My ads:** `GET /users/me/listings` с `withTrashed`, `deleted_at` в resource,
  `POST /listings/{uuid}/restore` — `SellerCabinetTest`.
- **§17 feature-flags** — `Done`: `GET /api/v1/public/feature-flags`
...
| 17 | Feature-flags (публичные) | `Done` | `GET /public/feature-flags` |
| 18 | Телефон в профиле | `Done` | `PATCH /users/me` `phone` |
| — | Восстановление объявления | `Done` | `POST /listings/{uuid}/restore` |
| — | PHPUnit PostgreSQL | `Done` | `modelizmclub_test`, 103 теста |

**Полностью на фронте, backend не нужен:** №6 (страница `/settings` — UI есть,
backend для кабинета закрыт), №8 (i18n лендинга), №12 (demo auth-гейты).

**По ранним пунктам (№1–14, §17–18, §26–27) открытых backend-задач нет** — на проде всё
включается, когда `isDemoMode()` false и есть валидный токен.

**Открытые backend-задачи** (только infra/deferred, backend-код не требуется):
...
**Метод:** DELETE  
**Response:** `{ "message": "ok" }`  
**Статус:** `Done` (2026-07-06, fix 2026-07-16) — `hideMessageForMe` в `chat.ts`, endpoint
`DELETE /conversations/{id}/messages/{uuid}` (soft hide через `message_user_hides`,
`ChatService::hideMessageForUser` → `DB::table(...)->updateOrInsert`). Тест
`ChatFrontendIntegrationTest::test_hide_message_for_current_user_only`.
...
## 17. Публичный feature-flags endpoint

**Статус:** `Done` (2026-07-12, подтверждено 2026-07-16) — `GET /api/v1/public/feature-flags`
→ `{ "data": { "communities_enabled": bool, "market_enabled": bool, "escrow_enabled": bool } }`.
White-list из `AdminSetting` / `FeatureFlagsSeeder`. Фронт: `featureFlags.ts` →
`useFeatureFlag()`. Тест `EntityRequestsAndIconsTest::test_feature_flags_include_market_and_escrow`.

_Историческая спецификация (до реализации):_

**Задача:** скрыть пункт меню «Сообщества» через
...
## 18. Поле телефона в профиле

**Статус:** `Done` (2026-07-12, подтверждено 2026-07-16) — `PATCH /users/me` принимает
`phone` (nullable string, max 20) в `UpdateProfileRequest` / `UserService`.
Тест `SellerCabinetTest::test_profile_accepts_phone_and_socials`. Регистрация
(`POST /auth/register`) — phone опционально не добавлялся; профиль — основной путь.
...
**Статус:** `Done` (2026-07-15, подтверждено 2026-07-16) — `IconAsset`, `POST /media purpose=icon`,
`GET/DELETE /admin/icon-assets`, публичный `GET /icon-overrides`, SVG-санитизация на сервере.
Тесты `EntityRequestsAndIconsTest` (16 тестов).
...
## 27. Заявки на создание Канала / Сообщества

**Статус:** `Done` (2026-07-15, подтверждено 2026-07-16) — полный стек: `POST /channels/apply`,
`POST /communities/apply`, `GET /me/entity-requests`, admin approve/reject для community и channel,
approve создаёт сущность + owner. Тесты `EntityRequestsAndIconsTest`.
...
## 28. Ссылка в уведомлении о новой записи ведёт в начало ленты (2026-09-08)

**Статус:** `Ожидает бэкенд`. Задача фронта: отдельная страница записи
`/post/{uuid}` (ветка `feature/post-page`).

**Что не так.** У уведомлений типа `subscription_posts` в
`notifications.data->>'link'` записано `/feed` — без идентификатора записи
(замерено по проду 07.09.2026). Читатель нажимает «новая запись у автора,
на которого вы подписаны» и попадает наверх ленты, где ищет её сам.

**Что нужно.** При создании уведомления `subscription_posts` строить ссылку
как `/post/{uuid}`, где `uuid` — uuid опубликованной записи (то же значение,
что отдаёт `PostResource.uuid` и принимает `GET /posts/{uuid}`).

| Поле | Было | Надо |
|---|---|---|
| `data.link` | `/feed` | `/post/{uuid}` |

Остальная форма уведомления не меняется: фронт читает `data.link` как есть и
переходит по нему (`routes/notifications.tsx`), никакого разбора на клиенте
нет.

**Уже разосланные уведомления.** Переписывать историю не требуется: `/feed`
остаётся рабочим адресом. Если решите обновить — это `UPDATE` по
`notifications` с `type = 'subscription_posts'`, и по правилу проекта он
делается сначала с `--dry-run`, а выборка проверяется независимым запросом
(см. CLAUDE.md, «Проверять до выкатки»).

**Фронт со своей стороны готов:** маршрут `/post/{uuid}` отвечает 404 на
несуществующую запись, подчиняется тем же правам доступа, что и лента
(`route.feed` в карте доступа), и старые ссылки `/feed?post={uuid}`
переадресуются на него.

**Демо/заглушка:** не требуется — до правки ссылка просто ведёт в ленту, как
и сейчас.

---

## Backend QA — PostgreSQL test suite (2026-07-16)

Изолированная тестовая БД **не prod** (`modelizmclub_test`). Prod-seed не мешает тестам.

```bash
bash deploy/scripts/setup-test-db.sh   # один раз
bash deploy/scripts/run-backend-tests.sh
```

| Модуль | Feature-тест | CRUD |
|---|---|---|
| Auth / consent / reset password | `AuthFlowTest` | register, login, reset |
| Listings / каталог | `ListingFilterAndExtrasTest` | filter, sort, favorite, AI suggest |
| Chat / messenger | `ChatFrontendIntegrationTest` | attachments, hide, pin, forward, block |
| Friends | `FriendModuleTest` | request, accept, remove |
| Feed / posts | `FeedModuleTest` | create, publish, moderate, comments |
| Channels | `ChannelPostMediaTest` | publish + feed mirror |
| Billing | `BillingModuleTest` | plans, stub/vtb/yookassa checkout |
| Seller cabinet | `SellerCabinetTest` | wallet, boost, restore, stats, 2FA paths |
| Admin / moderation | `AdminModuleTest`, `VideoUploadModerationTest` | queue, approve |
| Communities / icons / заявки | `EntityRequestsAndIconsTest` | apply, approve, reject, SVG |
| Delivery | `DeliveryIntegrationTest` | profile, quote, CDEK shipment |
| Escrow | `EscrowDealTest` | checkout |
| Public content | `ChatListingPublicModuleTest` | listings, faq |
| Reports / reactions | `ReportAndCommentReactionTest` | report, comment react |

**Итого:** 103 passed, 468 assertions (2026-07-16, PostgreSQL `modelizmclub_test`).
