      ...
      rollupOptions: {
        output: {
          manualChunks(id: string) {
             ...
             * Звонки — своим чанком.
             *
             * `lib/calls.ts` импортируют и экраны звонка (отложенные), и
             * мессенджер: двух потребителей из разных чанков достаточно,
             * чтобы Rollup поднял модуль в общий — то есть в главный. Одного
             * динамического импорта мало, нужно указать место явно. Звонок
             * начинается действием из мессенджера, до первого кадра ленты
             * этот код не нужен никому.
             *
             * Только сам модуль: если затянуть сюда и `components/calls/*`,
             * вместе с ними приедет LiveKit — чанк вырастает до 2 МБ, и
             * выигрыш превращается в проигрыш (проверено сборкой).
             */
            if (/[\\/]src[\\/]lib[\\/]calls\.ts$/.test(id)) return "calls";
            return undefined;
          },
        },
      },
