...
class AnonymizeUsersCommand extends Command
{
    protected $signature = 'users:anonymize
        ...
        {--force : обезличить даже учётку приёмки (is_qa_account)}
    ...
    public function handle(AuditService $audit): int
    {
        ...
        foreach ($this->argument('users') as $raw) {
            ...
            /*
             * Учётку приёмки не трогаем без явного `--force`.
             *
             * 25.09 обезличили все три — 1228, 1229, 1230, — и проверять
             * «без SMS / с SMS / с подпиской» стало нечем. Опознать их
             * потом было не по чему: имя затёрто, почта заменена. Признак
             * `is_qa_account` живёт отдельно и это повторить не даёт.
             */
            if ($user->is_qa_account && ! $this->option('force')) {
                $this->error(sprintf(
                    '%d (%s): это учётка приёмки — нужен --force, если правда хотите',
                    $user->id,
                    $user->email,
                ));
