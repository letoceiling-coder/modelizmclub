...
/**
 * Money side of a safe deal, held on the buyer's card by VTB.
 *
 * Two-stage acquiring maps onto the deal lifecycle: `registerPreAuth` when the
 * buyer checks out, `deposit` when the deal completes, `reverse` (uncaptured)
 * or `refund` (captured) when it is cancelled or refunded. Every RBS reply is
 * journalled in `safe_deal_gateway_events`.
 */
class SafeDealSettlementService
{
    ...
    /**
     * Registers a card hold and returns the row carrying the checkout URL.
     * The deal stays unpaid until VTB reports orderStatus 1 (authorized).
     */
    public function openHold(SafeDeal $deal, User $buyer, string $description, ?string $returnUrl = null): SafeDealIncomingPayment
    {
        ...
        $register = $this->client->registerPreAuth([
        ...
        if ($orderId === '' || ! $formUrl) {
            $incoming->update([
                ...
                'fail_reason' => 'registerPreAuth.do did not return orderId/formUrl',
                'failed_at' => now(),
            ]);

            throw new RuntimeException('Не удалось зарегистрировать холд в ВТБ.');
        }
        ...
        $this->journal($deal, $incoming, 'preauth.registered', $register);
    ...
    /** Captures the hold — call when the deal completes. */
    public function capture(SafeDealIncomingPayment $incoming): SafeDealIncomingPayment
    {
        ...
        if (! $incoming->rbs_order_id) {
            throw new RuntimeException('Холд ВТБ не зарегистрирован.');
        }
