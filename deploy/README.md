...
### Токены учётных записей приёмки

Лежат на машине разработчика, вне репозитория и вне `/tmp`:

```
~/.config/modelizmclub/qa-tokens.json      права 600
```

Прежний файл `~/qa-run-2026-09-12/tokens.json` вычистился 25.09 вместе с
временными файлами, и дымовой прогон остановился на полпути — токены
пришлось восстанавливать по памяти и сверять через `/users/me`. Новое
место чистку и перезагрузку переживает.

Состояния: `registered` — 1274 «Приёмка · без SMS», `verified` — 1275
«Приёмка · с SMS», `subscriber` — 1276 «Приёмка · с подпиской» (подписка
до 25.09.2027), `admin` — 1207 (с 24.09 модератор, не владелец).

Здесь стояли 1228, 1229 и 1230. Их обезличили и заблокировали 24.09 в
21:12 — почты стали `anonymized-*@removed.invalid`, войти нельзя. Нынешние
три завела 25.09 команда `qa:seed-accounts`, и они помечены
`is_qa_account`, чтобы `users:anonymize` не снёс их без `--force`: метка
появилась ровно после той истории.

Завести заново или перевыдать токены — одной командой. Существующие она не
пересоздаёт, а выдаёт им новый токен и печатает его:

```bash
php artisan qa:seed-accounts --dry-run   # посмотреть, ничего не меняя
php artisan qa:seed-accounts             # выдать токены и напечатать
```

Напечатанное кладут в `qa-tokens.json` руками — на экран оно попадает
один раз, в базе токен хранится хешем.

Проверить, что токен жив:

```bash
curl -s -H "Authorization: Bearer $(python3 -c 'import json;print(json.load(open("'"$HOME"'/.config/modelizmclub/qa-tokens.json"))["verified"])')" \
  https://api.modelizmclub.ru/api/v1/users/me | head -c 120
```

### Пароль учётных записей приёмки

Лежит рядом с токенами, вне репозитория:
...
Читают его все дымовые скрипты через общую библиотеку
`deploy/scripts/lib/qa-secrets.sh`. Переменная `SMOKE_PASSWORD`
(или `QA_PASSWORD` для `browser-qa.mjs`) перебивает файл — удобно для
разового прогона.

До 26.09 пароль был вписан строкой в десять скриптов, причём в половине
умолчанием вида `${SMOKE_PASSWORD:-…}`. Умолчание опаснее прямой записи:
пароль сменили, переменную не выставили — и прогон молча пробует старый,
а отчёт говорит «вход не работает». Теперь файла нет — скрипт
останавливается и объясняет, чего не хватает.

**`reset-demo-user.sh` до 26.09 вредил.** Он записывал зашитый пароль
живой учётке `demo@modelizmclub.ru`, то есть любой его запуск откатывал
смену пароля молча. Теперь он берёт пароль из того же файла: источник
один, возврата к неизвестно чему не происходит.

Создать файл:

```bash
mkdir -p ~/.config/modelizmclub
printf '{"password": "%s"}\n' 'ВАШ_ПАРОЛЬ' > ~/.config/modelizmclub/qa-secrets.json
chmod 600 ~/.config/modelizmclub/qa-secrets.json
```

### Обнуление балансов учётных записей приёмки

```bash
php artisan wallets:zero 1229 1230 --dry-run
php artisan wallets:zero 1229 1230 --actor=508 --reason='Обнуление перед запуском'
```

Проводки не удаляются: добавляется списание типа `admin_adjustment`, и
история остаётся. Кошелёк с непустым залогом команда не трогает и
останавливается — залог принадлежит незакрытой сделке, а не кошельку.

### Ключи Яндекс.Карт и Метрики — включаются без выкатки

Оба приходят с `/public/bootstrap` и хранятся в `system_settings` под
ключом `integrations.keys`. Вставка значения действует со следующей
загрузки страницы: кеш ответа сбрасывается событием модели настройки.

Через админку — раздел настроек, ключ `integrations.keys`, значение:

```json
{ "yandex_maps_key": "…", "metrika_id": "…" }
```

Из консоли, если админка недоступна:

```bash
cd /var/www/modelizmclub/backend
php artisan tinker --execute='\App\Models\SystemSetting::updateOrCreate(
  ["key" => "integrations.keys"],
  ["value" => ["yandex_maps_key" => "ВАШ_КЛЮЧ", "metrika_id" => "12345678"], "group" => "integrations"],
);'
```
...
```bash
curl -s https://api.modelizmclub.ru/api/v1/public/bootstrap | grep -ao '"integration_keys":{[^}]*}'
```

Номер счётчика чистится от постороннего при чтении: из кабинета Метрики
его копируют вместе с подписью и пробелами, а счётчик с мусором в номере
молча не заводится — заметить это можно было бы только по отсутствию
визитов через сутки.

`VITE_YANDEX_MAPS_KEY` и `VITE_METRIKA_ID` остаются запасным путём: если
в настройках пусто, а в сборке значение есть — берётся оно.

### Выкатка: скрипт и кеш маршрутов

`deploy/scripts/deploy-remote.sh` — копия того, что лежит на сервере в
`/root/deploy-remote.sh`. До 25.09 её в репозитории не было: скрипт,
которым выкатывается прод, нельзя было ни прочитать в ревью, ни
откатить, ни заметить, что он изменился.

```bash
ssh -p 2222 root@31.207.75.124 'bash /root/deploy-remote.sh <полный-sha>'
```

После правки скрипта на сервере — обновить копию здесь, и наоборот:

```bash
scp -P 2222 deploy/scripts/deploy-remote.sh root@31.207.75.124:/root/deploy-remote.sh
```

**Кеш маршрутов.** Раньше он пересобирался, только если в вызов
передавали вторым доводом слово `routes`. Не передавал никто: в каждом
журнале с 19.09 стоит `routes=no`, и кеш пролежал шесть дней. Новый путь
при живом кеше отвечает **404** — и это не похоже на «забыли
пересобрать», это выглядит как ошибка в коде.

Теперь скрипт смотрит сам: менялись файлы маршрутов — пересобирает, в
журнале появляется `route cache rebuilt`. Проверить руками:

```bash
ssh -p 2222 root@31.207.75.124 'cd /var/www/modelizmclub/backend && php artisan route:list --path=<кусок-пути>'
```

Пусто при живом коде — кеш устарел.
...
| Где | Ошибок tsc |
| --- | --- |
| `/var/www/modelizmclub/frontend` (основной чекаут) | **141** |
| свежий worktree до сборки | 124 |
| worktree деплоя после сборки | **39** — тот же порог, что на машине разработки |

Причин у разницы две, и обе про окружение, а не про код.

**Нет `src/routeTree.gen.ts`.** Его создаёт сборка, в git он не хранится. Без
него каждый импорт маршрута не разрешается, и tsc насчитывает лишнюю сотню
ошибок в файлах, где ничего не сломано. Отсюда 124 против 39.

**`frontend/node_modules` в основном чекауте устарел.** На 08.09 там 340
пакетов от 13 августа, в worktree деплоя — 519. Отсюда 141 против 124.

Правильный порядок — тот же, что у деплоя: собрать, потом проверять.
...
**Не запускайте `bun run build` в живом worktree** — том, на который смотрит
симлинк `frontend/.output`. Это ровно та авария, ради которой сборку и вынесли
в отдельное дерево: запрос, попавший в середину сборки, ловит удалённый chunk.
Куда смотрит симлинк сейчас: `readlink -f frontend/.output`.

Проверять tsc в живом worktree можно — `--noEmit` ничего не пишет.

**`frontend/node_modules` в основном чекауте рантайму не нужен.** Nitro
собирает сервер самодостаточным: в `.output/server/node_modules` лежит один
`tslib`, `server-graceful.mjs` импортирует только `node:net`, и работающий
процесс не держит ни одного файла и ни одной карты памяти под
`frontend/node_modules` (проверено по `/proc/<pid>`). Ни один скрипт деплоя к
нему не обращается. Это наследство до перехода на worktree-сборки; обновлять
его незачем, а если удалите — сломается только ручной запуск tsc в основном
чекауте, который и так даёт неверное число.
