...
#
# С 08.09 печать про соседние деревья стала запретом: обычный коммит из общего
# дерева, когда рядом уже заведено дерево с веткой, не проходит. Печати было
# мало — оба смёта случились при списке соседей на экране. Слияний запрет не
# касается: у них свой хук, и `git merge --no-ff` из общего дерева работает
# как работал. Своё дерево заводится одной командой:
#
#   bash deploy/scripts/worktree.sh new fix/<задача>
#
...
# 2. Обычный коммит из общего дерева при живой параллельной работе — отказ.
#
# Раньше здесь стояла печать: «рядом столько-то деревьев». Печати оказалось
# мало. 08.09 `git add -A` в общем дереве дважды смёл чужую незакоммиченную
# работу, и оба раза список соседей был на экране — его пролистали.
#
# Запретить `git add -A` git не даёт: точки входа «до add» у него нет. Зато
# можно запретить то, без чего вред не случается, — обычный коммит из общего
# дерева в тот момент, когда рядом уже кто-то работает. Оба смёта 08.09 были
# ровно этим.
#
# Мержи проверка не трогает: у слияния свой хук (`pre-merge-commit`), и
# привычный `git checkout master && git merge --no-ff` из общего дерева
# работает как работал.
#
# Соседом считается дерево с веткой. Временные деревья с отсоединённым HEAD
# (их заводит оснастка) параллельной работой не являются, и учитывать их
# значило бы запретить коммиты из общего дерева всегда — а запрет, который
# срабатывает всегда, за неделю превращается в привычку писать --no-verify.
#
# Слияние с конфликтом — исключение, и найдено оно опытом, а не рассуждением.
# `git merge` без конфликта зовёт `pre-merge-commit` и до `pre-commit` не
# доходит, а вот после разрешения конфликта коммит делается обычным
# `git commit` — и запрет сработал бы ровно на той операции, ради которой
# общее дерево и нужно. Проверено на отдельном репозитории: без конфликта
# хук молчит, с конфликтом — срабатывает.
MERGING=0
git rev-parse -q --verify MERGE_HEAD >/dev/null 2>&1 && MERGING=1
git rev-parse -q --verify CHERRY_PICK_HEAD >/dev/null 2>&1 && MERGING=1
...
if [[ "${ROOT}" == "${MAIN_WT}" && -n "${PEERS}" && "${MERGING}" == "0" ]]; then
  echo "" >&2
  echo "pre-commit: коммит из общего рабочего дерева, когда рядом идёт работа." >&2
  echo "" >&2
  while IFS= read -r wt; do
    [[ -z "${wt}" ]] && continue
    if [[ -d "${wt}" ]]; then
      WT_DIRTY="$(wt_status "${wt}" | wc -l | tr -d ' ')"
      echo "            ${wt} — незакоммичено: ${WT_DIRTY}" >&2
    else
      echo "            ${wt} — каталога нет" >&2
    fi
  done <<< "${PEERS}"
  echo "" >&2
  echo "            Общее дерево в это время — только для слияний." >&2
  echo "            Заведите своё:  bash deploy/scripts/worktree.sh new fix/<задача>" >&2
  echo "            Осознанно обойти: git commit --no-verify" >&2
  exit 1
fi

# 2а. Кто ещё работает — на глаза всегда, даже когда коммит разрешён.
...
#
# Индекс берётся настоящий. `git commit -- <пути>` подсовывает хуку временный,
# собранный из HEAD и перечисленных путей, и в нём всё добавленное про запас
# выглядит неотслеживаемым: файл, ждущий следующего коммита, показывался как
# «??», то есть как чужой мусор. С настоящим индексом он либо помечен «A» и
# отфильтрован как добавленный, либо его тут и нет.
REST="$(env -u GIT_INDEX_FILE git status --porcelain | grep -v '^[MARCD] ' || true)"
if [[ -n "${REST}" ]]; then
  REST_COUNT="$(printf '%s\n' "${REST}" | wc -l | tr -d ' ')"
