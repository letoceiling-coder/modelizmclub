...
**Неверно с 10.09.2026:** `.github/workflows/ci.yml` в корне есть и работает —
четыре работы на каждый push (гигиена, `tsc` + eslint, phpunit + эталон схемы,
composer audit). Выкатка без зелёного прогона запрещена правилом в `CLAUDE.md`.
Стоковый скелет в `backend/.github/` по-прежнему инертен.

~~**Три файла локализации — 10 160 строк.** `en.ts` 3446, `zh.ts` 3401,
`ru.ts` 3313.~~ **Неверно с 10.09.2026:** английский и китайский словари
удалены, осталось два файла — `ru.ts` 2624 и `ru-admin.ts` 1204. Это 22% всего кода `frontend/src`. Каждый разъезжается с
