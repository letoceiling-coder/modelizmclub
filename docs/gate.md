...
| `route.channels`, `layout.nav.channels` | auth | subscription | Каналы закрыты подпиской по решению заказчика. |
...
Открытый маршрут не открывает действий: «Вступить» и «Открыть чат» идут
через `requirePremium` (ступень `subscriber`), жалоба — через
`requireAccount`, мастер создания сообщества сам разворачивает гостя на
`requireAccount`. Гость на любом из них видит окно входа — `firstFailingStep`
никогда не покажет ему paywall.
