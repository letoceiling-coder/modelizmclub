...
## `CategoryTaxonomyTest` сравнивает идентификаторы разных таблиц
...
**Как чинить:** развести последовательности по построению — добивать
`listing_categories` строками, пока её следующий идентификатор не уйдёт выше
наибольшего у `post_categories`, — и только потом создавать пару. Тогда
утверждение держится всегда, а не при удачном порядке.
